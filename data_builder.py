...
def random_lines(filename, count=1):    
    temp = []
    with open(filename) as f:
        lines = f.read().splitlines()
        temp = random.sample(lines, count)
    return temp
...
data = {"Previous Close":[],"Open":[],"Bid":[],"Ask":[],"Day's Range":[],"52 Week Range":[],"Volume":[],"Avg. Volume":[],"Market Cap":[],"Beta (5Y Monthly)":[],"PE Ratio (TTM)":[],"EPS (TTM)":[],"Earnings Date":[],"Forward Dividend & Yield":[],"Ex-Dividend Date":[],"1y Target Est":[],"ticker":[],"url":[]}
#tickers = random_lines('ticker.txt', 10289)
tickers = read_file('ticker.txt')
...
for ticker in tickers:
    ...
    if i%100 == 0:
        ...
        temp.to_csv('data'+str(i)+'.csv', sep=',')
